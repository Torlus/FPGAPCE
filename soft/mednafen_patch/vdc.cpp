...
void VDC_SetPixelFormat(int rshift, int gshift, int bshift)
{
 ...
 printf("rshift = %d, gshift = %d, bshift = %d\n", rshift, gshift, bshift);
 ...
 for(int x = 0; x < 512; x++)
  ...
  //GE the conversion will be performed after rendering to systemColorMaps
  // so the systemColorMaps will be used as reverse-LUTs to our custom 256-color palette
  // the lowest bit of the Blue component will be lost, and the palette is doubled for convenience
  printf("*GE* VDC_SetPixelFormat\n");
  for(int i = 0; i < 512; i++) {
	int r,g,b;
	b = (i & 0x007)<<5;
	r = (i & 0x038)<<2;
	g = (i & 0x1C0)>>1;
	cap_pal[i>>1] = (r << 16) | (g << 8) | (b << 0);
  }
	for(int i = 0; i < 512; i++) {
		printf("%03X %08X = %02X %06X\n", i, systemColorMap32[i], i>>1, cap_pal[i>>1]);
	}
}
 ...
 */
 ...
 if (cap_request) {
	...
	printf("*GE* VDC_RunFrame - capture request\n");
	cap_request = 0;

 	FILE *fp = fopen("screen.bmp", "wb");
	InitBMPHeader(&bmp_header, 576, 242, 8);
	fwrite(&bmp_header, sizeof(struct bmp_header), 1, fp);
	for (int i = 0; i < 256; i++)
		fwrite(&cap_pal[i], sizeof(UINT32), 1, fp);
	for(int y = 242-1; y >= 0; y--) {
		for(int x = 0; x < 576; x++) {
			uint32 col = *( pXBuf + (y*1024) + x );
			col = col & 0x00ffffff;
			unsigned char p = 0;
			int found = 0;
			for(int k = 0; k < 512; k++) {
				if (systemColorMap32[k] == col) {
					p = (k>>1);
					found = 1;
					break;
				}
	...
	fclose(fp);
	fp = fopen("screen.bin", "wb");
	// RAMs (VDC,VCE)
	fwrite(vdc->VRAM, sizeof(uint16), 0x8000, fp);
	fwrite(vdc->SAT, sizeof(uint16), 0x100, fp);
	
	fwrite(vce.color_table, sizeof(uint16), 0x200, fp);
	// Registers (VDC,VCE)
	fwrite(&vdc->MAWR, sizeof(uint16), 1, fp);
	fwrite(&vdc->MARR, sizeof(uint16), 1, fp);
	fwrite(&vdc->CR, sizeof(uint16), 1, fp);
	fwrite(&vdc->RCR, sizeof(uint16), 1, fp);
	fwrite(&vdc->BXR, sizeof(uint16), 1, fp);
	fwrite(&vdc->BYR, sizeof(uint16), 1, fp);
	fwrite(&vdc->MWR, sizeof(uint16), 1, fp);
	fwrite(&vdc->HSR, sizeof(uint16), 1, fp);
	fwrite(&vdc->HDR, sizeof(uint16), 1, fp);
	fwrite(&vdc->VSR, sizeof(uint16), 1, fp);
	fwrite(&vdc->VDR, sizeof(uint16), 1, fp);
	fwrite(&vdc->VCR, sizeof(uint16), 1, fp);
	fwrite(&vdc->DCR, sizeof(uint16), 1, fp);
	fwrite(&vdc->SOUR, sizeof(uint16), 1, fp);
	fwrite(&vdc->DESR, sizeof(uint16), 1, fp);
	fwrite(&vdc->LENR, sizeof(uint16), 1, fp);
	fwrite(&vdc->SATB, sizeof(uint16), 1, fp);
	
	fwrite(&vce.CR, sizeof(uint8), 1, fp);
	fclose(fp);
 }
}
